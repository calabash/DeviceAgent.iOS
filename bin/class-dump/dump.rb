...
FRAMEWORKS=["XCTestCore", "XCTAutomationSupport", "DTXConnectionServices", "XCUIAutomation"]
...
FRAMEWORKS_MAP.each do |framework, output_path|
  ...
  elsif framework == "XCUIAutomation"
    binary_path = File.join(library, "PrivateFrameworks",
...
FRAMEWORKS_MAP.each do |_, output_path|
  ...
  ["NSCoding-Protocol.h",
   ...
   "NSMutableCopying-Protocol.h"].each do |header|
  ...
  Dir.glob(File.join(output_path, "*.h")) do |header|
    ...
    originals.each do |line|
      if !LINES_TO_REMOVE.include?(line.chomp)
        ...
        if line[/<(XCTAutomationSupport|XCTestCore|XCUIAutomation)\/.+.h>/]
...
# Use the version of CDStructures.h in XCTestCore/
FileUtils.rm(File.join(FRAMEWORKS_MAP["XCTAutomationSupport"], "CDStructures.h"))
FileUtils.mv(File.join(FRAMEWORKS_MAP["XCTestCore"], "CDStructures.h"),
             File.join(FRAMEWORKS_MAP["XCTestCore"], ".."))
...
UMBRELLA_HEADER = File.join(FRAMEWORKS_MAP["XCTestCore"], "..", "CBX-XCTest-Umbrella.h")
File.open(UMBRELLA_HEADER, "w:UTF-8") do |file|
  file.puts("// Generated by bin/class-dump/dump.rb")
  file.puts("")
  file.puts("#if LOAD_XCTEST_PRIVATE_HEADERS")
  headersXCTestCore = Dir.glob(File.join(FRAMEWORKS_MAP["XCTestCore"], "*.h")).sort

  headersXCTestCore.each do |header|
    file.puts(%Q[#import "XCTestCore/#{File.basename(header)}"])
  end

  headersXCUIAutomation = Dir.glob(File.join(FRAMEWORKS_MAP["XCUIAutomation"], "*.h")).sort
  headersXCUIAutomation.each do |header|
    file.puts(%Q[#import "XCUIAutomation/#{File.basename(header)}"])
  end
